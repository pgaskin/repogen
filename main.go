...
func main() {
	...
	origin := pflag.StringP("origin", "o", "reposerver", "sets the origin field used in the Release file (this field is used as a user-friendly way to identify the repository)")
	description := pflag.StringP("description", "d", "Generated by reposerver", "sets the description field used in the Release file")
	generateContents := pflag.BoolP("generate-contents", "c", false, "generates the Contents index (makes reposerver slower to load)")
	...
	if *help || pflag.NArg() != 3 {
		fmt.Fprintf(os.Stderr, "Usage: reposerver [OPTIONS] PRIVATE_KEY_FILE INPUT_DIR OUTPUT_DIR\n\nOptions:\n")
		pflag.PrintDefaults()
